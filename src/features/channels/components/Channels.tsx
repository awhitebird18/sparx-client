...
const Channels: React.FC = () => {
  ...
  return (
    <Content>
      ...
      <ul className="space-y-3 overflow-auto flex-1 mt-2 pr-2">
        {isLoading ? (
          <div className="mt-10">
            <Spinner />
          </div>
        ...
        {!isLoading && displayedChannels.length
          ? displayedChannels.map((channel) => (
              <li
                key={channel.uuid}
                className="flex justify-between items-center border border-border p-4 cursor-pointer rounded-md group hover:bg-secondary"
