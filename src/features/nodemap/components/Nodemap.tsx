...
const NodeMap: React.FC = observer(() => {
  ...
  const { fetchUserChannelData, channels, fetchChannelUserCounts } = useStore('channelStore');
  const { currentWorkspaceId, nodemapState } = useStore('workspaceStore');
  const { fetchNodemapSettingsApi, isEditing, setIsEditing } = useStore('nodemapStore');
