...
const Message = ({ message, showUser }: { message: Message; showUser: boolean }) => {
  return (
    <div
      key={message.uuid}
      className={
        'message px-2 rounded-lg hover:bg-accent dark:hover:bg-accent py-1.5 overflow-hidden'
      }
    >
      <div className="flex gap-2">
        {showUser ? (
          <Avatar className="h-10 w-10">
            ...
            <AvatarFallback
              className="opacity-40 h-full w-full rounded-sm border bg-red-200"
        ...
        <div className={`flex flex-col ${showUser ? 'h-10' : 'h-6'} w-full overflow-hidden`}>
          {showUser ? (
